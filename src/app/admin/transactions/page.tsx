  ...
  const fetchTransactions = async () => {
    try {
      ...
      const { data: orders, error: ordersError } = await supabase
        .from('orders')
        .select('*, users(name), bundles(name)')
        .eq('status', 'completed');
      
      if (ordersError) throw ordersError;

      // 2. Fetch approved withdrawals
      ...
      const mappedOrders = ((orders || []) as unknown as DBOrder[]).map((o) => ({
        ...
        details: `Sold ${o.bundles?.name || 'Data Bundle'} to ${o.customer_phone}`,
        amount: Number(o.customer_paid),
        fee: Number(o.platform_fee),
      ...
      const mappedWithdrawals = ((withdrawals || []) as unknown as DBWithdrawal[]).map((w) => {
        ...
        return {
          ...
          type: 'withdrawal',
          agent: w.users?.name || 'Agent Partner',
          details: `Payout to ${w.momo_number} (${w.network})`,
      ...
      const allTxns = [...mappedOrders, ...mappedWithdrawals].sort((a, b) => {
