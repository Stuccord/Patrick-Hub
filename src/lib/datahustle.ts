...
export interface DataHustleBalance {
  success: boolean;
  balance: number;
  currency: string;
}
...
/**
 * Fetch DataHustle wallet balance.
 */
export async function getDataHustleBalance(): Promise<DataHustleBalance> {
  const apiKey = process.env.DATAHUSTLE_API_KEY;
  if (!apiKey) {
    throw new Error('DATAHUSTLE_API_KEY is not configured');
  }
  ...
  try {
    const res = await fetch('https://api.datahustle.shop/api/developer/balance', {
      method: 'GET',
      headers: {
        'X-API-Key': apiKey,
      },
    ...
    if (res.ok && data.status === 'success') {
      return {
        success: true,
        balance: Number(data.data?.walletBalance ?? 0),
    ...
    console.error('[DataHustle] Balance error:', err);
    return {
      success: false,
      balance: 0,
      currency: 'GHS',
